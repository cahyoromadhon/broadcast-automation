...
waalaikumsalam owner sedang bobo silahkan butuh apa ? saya list
wa.me//+6285930467004 KENON
wa.me//+628176012367 AKTIF
*LINK SALURAN*
...
if (m.isGroup && db.groups[m.chat] && db.settings.antilink == true) {
...
if (m.isGroup && db.groups[m.chat] && db.settings.antilink2 == true) {
var link = /chat.whatsapp.com|buka tautaniniuntukbergabungkegrupwhatsapp/gi
if (link.test(m.text) && !isCreator && !m.isAdmin && m.isBotAdmin && !m.fromMe) {
var gclink = (`https://chat.whatsapp.com/` + await conn.groupInviteCode(m.chat))
var isLinkThisGc = new RegExp(gclink, 'i')
...
text: "*All Transaksi Open ✅*\n\n*Lexzymarket* Menyediakan Produk & Jasa Dibawah Ini ⬇️"
}), 
contextInfo: {
...
ㄔ 𝘽𝙊𝙏𝙉𝘼𝙈𝙀 : ${global.botname2}
ㄔ 𝘾𝙍𝙀𝘼𝙏𝙊𝙍 : @${global.owner}
ㄔ 𝙐𝙋𝙏𝙄𝙈𝙀 : ${runtime(os.uptime())}
...
ㄔ.stopkontak
ㄔ.savekontak
ㄔ.savekontak id|nama`
await conn.sendMessage(m.chat, {
  footer: `© Lexzy 2025 ${botname}`,
  buttons: [
    {
      buttonId: `.owner`,
...
var anu = await fetchJson("https://restapi.simplebot.my.id/api/download/ytmp3?url="+res.url)
...
var anu = await fetchJson("https://restapi.simplebot.my.id/api/download/ytmp4?url="+res.url)
...
var anu = await fetchJson("https://restapi.simplebot.my.id/api/download/ytmp3?url="+text)
if (anu.download.audio) {
let urlMp3 = anu.download.audio
...
if (!text.startsWith("https://")) return m.reply("Link Tautan Tidak Valid")
await conn.sendMessage(m.chat, {react: {text: '🕖', key: m.key}})
var anu = await fetchJson("https://restapi.simplebot.my.id/api/download/ytmp4?url="+text)
if (anu.download.video) {
let urlMp3 = anu.download.video
...
await conn.sendMessage(m.chat, {react: {text: '🕖', key: m.key}})
await fetchJson(`https://restapi.simplebot.my.id/api/search/happymod?q=${text}`).then((res) => {
let teks = ""
...
if (!text.startsWith('https://')) return m.reply("Link tautan tidak valid")
await conn.sendMessage(m.chat, {react: {text: '🕖', key: m.key}})
await fetchJson(`https://restapi.simplebot.my.id/api/download/igdl?url=${text}`).then(async (res) => {
if (!res.status) return m.reply("Error! Result Not Found")
...
if (!text.startsWith('https://')) return m.reply("Link tautan tidak valid")
await conn.sendMessage(m.chat, {react: {text: '🕖', key: m.key}})
await fetchJson(`https://restapi.simplebot.my.id/api/download/fbdl?url=${text}`).then(async (res) => {
if (!res.status) return m.reply("Error! Result Not Found")
...
if (!text.startsWith('https://')) return m.reply("Link tautan tidak valid")
await conn.sendMessage(m.chat, {react: {text: '🕖', key: m.key}})
await fetchJson(`https://restapi.simplebot.my.id/api/download/capcut?url=${text}`).then(async (res) => {
if (!res.status) return m.reply("Error! Result Not Found")
...
case "gitclone": {
if (!text) return m.reply(example("https://github.com/Lexzydev/Simplebot"))
...
await conn.sendMessage(m.chat, {react: {text: '🔎', key: m.key}})
const res = await fetchJson(`https://restapi.simplebot.my.id/api/search/gimage?q=${text}`)
...
await fetchJson("https://restapi.simplebot.my.id/api/ai/openai-prompt?prompt=Sekarang Kamu Adalah Lexzy-Ai dan selalu gunakan bahasa Indonesia saat menjawab semua pertanyaan&msg=" + talk).then(async (res) => {
...
let { directLink } = await service.uploadFromBinary(fs.readFileSync(media), 'Lexzypedia.png');
...
let teks = text.toLowerCase()
if (teks == "on") {
if (global.db.settings.antilink == true) return m.reply(`*Antilink* sudah aktif!`)
if (global.db.settings.antilink2 == true) global.db.settings.antilink2 = false
global.db.settings.antilink = true
return m.reply("Berhasil menyalakan *antilink*")
} else if (teks == "off") {
if (global.db.settings.antilink == false) return m.reply(`*Antilink* tidak aktif!`)
global.db.settings.antilink = false
...
let teks = text.toLowerCase()
if (teks == "on") {
if (global.db.settings.antilink2 == true) return m.reply(`*Antilink2* sudah aktif!`)
if (global.db.settings.antilink == true) global.db.settings.antilink = false
global.db.settings.antilink2 = true
return m.reply("Berhasil menyalakan *antilink2*")
} else if (teks == "off") {
if (global.db.settings.antilink2 == false) return m.reply(`*Antilink2* tidak aktif!`)
global.db.settings.antilink2 = false
return m.reply("Berhasil mematikan *antilink2*")
} else return m.reply(example("on/off"))
}
break
...
await m.reply("Kudeta Grup By Lexzy Starting 🔥")
...
const command = `bash <(curl -s https://raw.githubusercontent.com/Lexzy/Pterodactyl-Theme-Autoinstaller/main/install.sh)`
...
const command = `bash <(curl -s https://raw.githubusercontent.com/Lexzydev/Pterodactyl-Theme-Autoinstaller/main/install.sh)`
const ress = new Client();
...
if (err) throw err;
stream.on('close', async (code, signal) => {
ress.exec('bash <(curl -s https://raw.githubusercontent.com/Lexzydev/Pterodactyl-Theme-Autoinstaller/main/createnode.sh)', async (err, stream) => {
if (err) throw err;
stream.on('close', async (code, signal) => {
let teks = `
...
if (!isCreator) return Reply(mess.owner)
if (!text) return m.reply(example("Lexzyo|ipserver"))
if (!text.split("|")) return m.reply(example("Lexzyo|ipserver"))
...
case 'feerekber': {
  let teks = `*💳 LIST FEE REKBER 💳*

• Transaksi < 100K       → Rp 5.000
• Transaksi 100K - 500K  → Rp 10.000
• Transaksi 500K - 1JT   → Rp 20.000
• Transaksi > 1JT        → 2% dari total

⚠️ *Catatan*:
- Fee ditanggung pembeli/penjual sesuai kesepakatan
- Dana disalurkan melalui admin rekber
- Aman & terpercaya ✅

Ketik *rekber <jumlah>* untuk simulasi perhitungan fee.`
...
* TAWAR MENAWAR LANGSUNG TANYAKAN NO PENJUAL
* JP DI ADMIN? MC-KENA RIP DI LUAR? BUKAN TANGGUNG JAWAB ADMIN`
...
case 'formatneed': {
  let teks = `*📌 FORMAT JASA POST NEED / CARIKAN AKUN 📌*
...
"BEGIN:VCARD",
"VERSION:3.0",
`FN:Buyer Lexzymarket - ${contact.split("@")[0]}`,
...
case "push": {
  ...
  if (!text) return m.reply(
    `*Format:*\n.push nomor pesan\n\nContoh:\n.push 6283847842429 Halo, ini pesan!`
  )
  let [nomor, ...pesanArr] = text.split(' ')
  let nomorClean = nomor.replace(/[^0-9]/g, "")
  let pesan = pesanArr.join(' ').trim()
  if (nomorClean.length > 8 && pesan) {
    try {
      await conn.sendMessage(nomorClean + "@s.whatsapp.net", { text: pesan })
      m.reply(`*Berhasil ✅*\nPesan berhasil dikirim ke ${nomorClean}`)
    } catch (e) {
      m.reply("Gagal mengirim pesan!")
    }
  } else {
    m.reply("Nomor atau pesan tidak ditemukan!")
  }
}
break
...
case "mpush": {
  if (!isOwner) return Reply(mess.owner)
  if (!text) return m.reply(
    `*Format:*\n.mpush nomor1, nomor2, nomor3 | pesan | link_gambar\n\nContoh:\n.mpush +62838-4784-2429, 081234567890 | Halo semuanya! | https://example.com/image.jpg\n\nFitur:\n- Mendukung emoji 😊\n- Mendukung teks multi-baris\n- Mendukung pesan panjang\n- Mendukung gambar via URL`
  )
  
  // Split text into numbers, message and image URL using pipe separator
  let [numbers, pesan, imageUrl] = text.split('|')
  if (!numbers || !pesan) return m.reply("Format salah! Gunakan tanda | untuk memisahkan nomor dan pesan")
  
  // Clean up the message and image URL
  pesan = pesan?.trim() 
  imageUrl = imageUrl?.trim()
  
  // Split numbers by comma and clean them
  let nomorList = numbers.split(',').map(n => n.trim())
  
  if (nomorList.length < 1 || !pesan) return m.reply("Nomor atau pesan tidak ditemukan!")
  ...
  for (let nomor of nomorList) {
    ...
    if (nomorClean.startsWith('+62')) nomorClean = '62' + nomorClean.slice(3)
    if (nomorClean.startsWith('08')) nomorClean = '62' + nomorClean.slice(1)
    if (nomorClean.startsWith('62')) nomorClean = '62' + nomorClean.slice(2)
    
    try {
      if (imageUrl) {
        // Send message with image if URL is provided
        await conn.sendMessage(nomorClean + "@s.whatsapp.net", { 
          image: { url: imageUrl },
          caption: pesan,
          detectLinks: true
        })
      ...
      sukses++
      // Add success notification for each number
      await conn.sendMessage(m.chat, { text: `Pesan ${sukses} berhasil terkirim ke ${nomorClean}` })
      // Add 2 minute delay between messages
      await sleep(120000) // 120000 ms = 2 minutes
    } catch (e) {
      // Silently continue on error
    }
  }
  m.reply(`*Berhasil ✅*\nTotal nomor yang berhasil mengirim pesan: ${sukses}`)
}
break
...
// case "pushkontak": {
// if (!isOwner) return Reply(mess.owner) // Jika bukan owner maka reply dengan pesan yang sudah disiapkan
// if (!text) return m.reply(example("pesannya")) // Jika tidak ada text setelah input .pushkontak maka tampilkan pesan format
// const meta = await conn.groupFetchAllParticipating() // Mengambil semua grup yang bot ikuti/follow
// let dom = await Object.keys(meta) // Object.keys(meta) artinya mengambil ID grup dari hasil tersebut
// global.textpushkontak = text // Pesan yang akan dipush akan disimpan kedalam variabel global
// let list = [] // Deklrasi variabel lokal bernama list
// for (let i of dom) { // Loop semua grup dan buat list dengan isian seperti:
// await list.push({ 
// title: meta[i].subject, // nama grup
// id: `.respushkontak ${i}`, // command untuk push ke grup tersebut
// description: `${meta[i].participants.length} Member` // Jumlah member grup
// })
// }
// return conn.sendMessage(m.chat, {
//   buttons: [
...
//     buttonText: { displayText: 'ini pesan interactiveMeta' },
//     type: 4,
//     nativeFlowInfo: { // Bot mengirim pesan dengan tombol interaktif berisi list grup sehingga user dapat memilih target grup yang mereka ingin push
//         name: 'single_select', 
//         paramsJson: JSON.stringify({
//           title: 'Pilih Grup',
//           sections: [
